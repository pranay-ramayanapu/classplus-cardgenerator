...
export async function renderCardToBlob(data) {
  const canvas = await renderCardToCanvas(data);
...
export async function shareBlobImage(blob, title = "Greeting Card") {
  ...
  return {
    mode: "fallback",
    reason: "Native share sheet is not available on this browser/device.",
  };
}
...
export async function shareCardImage(data, title = "Greeting Card") {
  ...
  return shareBlobImage(blob, title);
}

export function downloadBlobAsPng(blob, fileName = "greeting-card.png") {
  const imageUrl = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = imageUrl;
  link.download = fileName;
