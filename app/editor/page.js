      ...
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
        <section className="rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-soft backdrop-blur dark:border-slate-800 dark:bg-slate-950/80 sm:p-8">
                  ...
                  {group.items.map((template) => {
                    ...
                    return (
                      <button
                        ...
                        className={`group overflow-hidden rounded-2xl border bg-white text-left shadow-sm transition hover:-translate-y-1 hover:shadow-lg dark:bg-slate-900 ${
                          active
                            ? "border-sky-400 ring-4 ring-sky-100 dark:ring-sky-950/40"
                            : "border-slate-200 dark:border-slate-800"
                        }`}
                      >
                        <div className="relative aspect-[4/5] overflow-hidden">
                          <img src={template.image} alt={template.title} className="h-full w-full object-cover transition duration-300 group-hover:scale-105" />
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            <PositionChip label="Text position" value={`${activeTemplate.textPosition.top}, ${activeTemplate.textPosition.left}`} />
            <PositionChip label="Profile position" value={`${activeTemplate.profilePosition.top}, ${activeTemplate.profilePosition.left}`} />
          </div>
